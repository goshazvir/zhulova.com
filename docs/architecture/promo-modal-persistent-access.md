...
- **Header** (`Header.astro`): a `#header-gift-cta` pill button as the rightmost element of the desktop header row, after the social-icon block, wired via the existing inline `<script>` (same pattern as `#mobile-menu-button`). `showGiftCta` is also passed down to `MobileMenu` as a prop. Since GEO-45, nav, social icons and the gift button are nested in one `hidden md:flex` right-aligned cluster (instead of each carrying its own `hidden md:flex`/`hidden md:inline-flex`), so their spacing is explicit rather than computed by the outer row's `justify-between` across four siblings.
